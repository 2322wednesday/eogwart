class PersonalTest {
    constructor(target) {
        ...
        this.questions = {
            GHRS: [
                ...
                { question: '늦은 밥 거리를 홀로 것던 당신은 마법과 관련된 것 같은 기이한 울음소리를 듣게 된다. 이때 당신의 선택은?', answer: { a:'지팡이를 한 손에 쥐고, 경계 태세를 하며 조심스레 걸어나간다', b: '지팡이를 꺼내들고 소리의 출처를 찾아본다', c:'지팡이를 꺼내들고 그 자리에 서서 맞선다', d:'문제 발생을 대비해 마음속으로 효과적 방어주문을 떠올리며 눈에 띄지 않는 곳으로 물러난다'}},
        ...
        this.resultImages = {
            굴러온도르: './src/images/gryffin.jpeg', // 'G' 결과에 대한 이미지 URL
            훌라후프: './src/images/huffle.jpeg', // 'H' 결과에 대한 이미지 URL
            레진치료: './src/images/raven.jpeg',   // 'R' 결과에 대한 이미지 URL
            술이된인: './src/images/slytherin.jpeg'    // 'S' 결과에 대한 이미지 URL
        };        
        this.responseTimes = {};
        this.init();
    }
    ...
    finishAnswer(questionType, answer) {
        ...
        // 응답 시간을 저장
        if (!this.responseTimes[questionType]) {
            this.responseTimes[questionType] = [];
        }
        this.responseTimes[questionType].push(responseTime);
    ...
    createPersonalResult(totalResult, averageResponseTimes) {
        ...
        const resultMapping = { a: '굴러온도르', b: '훌라후프', c: '레진치료', d: '술이된인' }; // 결과 매핑
        ...
        return Object.keys(totalResult).reduce((acc, cur) => {
            ...
            // 최대 가중치 값을 가진 결과 유형을 찾습니다.
            const maxKey = Object.keys(weightedResult).reduce((a, b) => weightedResult[a] > weightedResult[b] ? a : b);

            return acc + resultMapping[maxKey];
        }, "");
    }
    ...
    render() {
        ...
        if (this.page === 0) {
            ...
            setTimeout(() => {
                ...
                const calcResult = this.calcResult(); // 결과 계산
                ...
                resultInforImg.src = this.resultImages[calcResult];
                console.log(calcResult);
                console.log(this.resultImages[calcResult]);
                resultTextElement.innerHTML = `당신의 기숙사는 <span class="point_text">${calcResult}</span>입니다.`;
